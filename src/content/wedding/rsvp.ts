...
const DIETARY_RESTRICTIONS_LABEL = 'Do you or anyone in your party have dietary restrictions?';
const OTHER_COMMENTS_LABEL = 'Anything else the bride & groom should know?';

export const RSVP_OPTIONS = ['I’ll be there!', 'Can’t attend'];

export const ID_TO_DATA: Record<FormId, { label: string, name: string, required: boolean }> = {
  name: { label: NAME_LABEL, name: 'entry.845038701', required: true },
  welcomeParty: { label: WELCOME_PARTY_LABEL, name: 'entry.2052144975', required: true },
  rsvp: { label: RSVP_LABEL, name: 'entry.1851412699', required: true },
  dietaryRestrictions: { label: DIETARY_RESTRICTIONS_LABEL, name: 'entry.1658189941', required: false },
  otherComments: { label: OTHER_COMMENTS_LABEL, name: 'entry.1596816796', required: false },
};
